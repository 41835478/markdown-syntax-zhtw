...
Markdown 支持两种标题的语法，[Setext] [1] 和 [atx] [2] 形式。Setext 形式是用底线的形式，利用 `=` （最高级标题）和 `-` （第二级标题），Atx 形式在行首插入 1 到 6 个 `#` ，对应到标题 1 到 6 阶。
...
Markdown 支持两种形式的连结语法： *行内* 和 *参考* 两种形式，两种都是使用角括号来把文字转成连结。
